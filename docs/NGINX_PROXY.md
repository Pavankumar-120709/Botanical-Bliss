...
This document explains the nginx reverse proxy setup that serves as the single entry point for the Botanical Bliss application.
...
```javascript
// botanical-bliss-ui/src/config/api.js
...
2. Test direct backend connection (internal):
   ```bash
   docker exec botanical-bliss-nginx curl http://backend:8080/api/v1/actuator/health
   ```
...
# Resource usage
docker stats botanical-bliss-nginx
